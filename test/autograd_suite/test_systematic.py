# -*- coding: utf-8 -*-
"""Ported from autograd ``tests/test_systematic.py`` (MIT) via mechanical import
rewrites. Op-coverage sweep; tests for ops pycograd lacks a rule for (or that
pass complex inputs) are marked skip -- see REPORT.md."""
...
from .numpy_utils import (
    binary_ufunc_check,
    binary_ufunc_check_no_same_args,
    stat_check,
    unary_ufunc_check,
)
...
def test_remainder():
    binary_ufunc_check_no_same_args(
        np.remainder, lims_A=[-0.9, 0.9], lims_B=[0.7, 1.9], test_complex=False
    )
...
def test_transpose():
    combo_check(np.transpose, [0])(
        [R(2, 3, 4)],
        axes=[None, [0, 1, 2], [0, 2, 1], [2, 0, 1], [2, 1, 0], [1, 0, 2], [1, 2, 0]],
    )
...
def test_repeat():
    combo_check(np.repeat, [0])(
        [R(2, 3, 4), R(3, 1)], repeats=[0, 1, 2], axis=[None, 0, 1]
    )
...
def test_tensordot_4():
    combo_check(np.tensordot, [0, 1], order=3)(
        [R(2, 2), R(4, 2, 2)], [R(2, 2), R(2, 2, 4)], axes=[1, 2]
    )
...
def test_atleast_3d():
    combo_check(np.atleast_3d, [0])(
        [1.2, R(1), R(7), R(1, 4), R(2, 4), R(2, 4, 5), R(2, 4, 3, 5)]
    )
...
def test_einsum_matmult():
    combo_check(np.einsum, [1, 2])(
        ["ij,jk->ik"], [R(2, 3), C(2, 3)], [R(3, 4), C(3, 4)]
    )


def test_einsum_matmult_broadcast():
    combo_check(np.einsum, [1, 2])(
        ["...ij,...jk->...ik"], [R(2, 3), R(2, 2, 3)], [R(3, 4), R(2, 3, 4)]
    )
...
def test_einsum_ellipses_tail():
    combo_check(np.einsum, [1, 2])(
        ["jk...,lj...->lk..."], [R(3, 2), R(3, 2, 4)], [R(2, 3), R(2, 3, 4)]
    )


def test_einsum_ellipses_center():
    combo_check(np.einsum, [1, 2])(
        ["j...k,lj...->lk..."], [R(2, 2), R(2, 2, 2)], [R(2, 2), R(2, 2, 2)]
    )


def test_einsum_three_args():
    combo_check(np.einsum, [1, 2])(
        ["ijk,lji,lli->lki"], [R(3, 4, 4)], [R(4, 4, 3)], [R(4, 4, 3)]
    )
...
def test_einsum2_covsum():
    combo_check(np.einsum, [0, 2])(
        [R(3, 4, 4)], [(0, 1, 2)], [R(4, 4, 3)], [(3, 1, 0)], [(3, 2, 0)]
    )
