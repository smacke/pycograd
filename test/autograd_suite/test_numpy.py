# -*- coding: utf-8 -*-
"""Ported from autograd ``tests/test_numpy.py`` (MIT) via mechanical import
...
def test_min_3_way_equality():
    def fun(x):
        return np.min(
            np.array([[x, x, x], [x, 0.5, 0.5], [0.5, 0.5, 0.5], [x, x, 0.5]]), axis=0
        )
