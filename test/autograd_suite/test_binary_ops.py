# -*- coding: utf-8 -*-
"""Ported from autograd ``tests/test_binary_ops.py`` (MIT). Operator and ufunc gradients
across scalar/vector/matrix broadcasting shapes."""
...
from ._compat import grad, value_and_grad
...
def test_pow():
    fun = lambda x, y: x**y
    make_positive = lambda x: np.abs(x) + 1.1
    for arg1, arg2 in arg_pairs():
        arg1 = make_positive(arg1)
        check_grads(fun)(arg1, arg2)


@pytest.mark.skip(reason="pycograd-gap: no VJP rule for np.arctan2")
def test_arctan2():
    for arg1, arg2 in arg_pairs():
        check_grads(np.arctan2)(arg1, arg2)


@pytest.mark.skip(reason="pycograd-gap: no VJP rule for np.hypot")
def test_hypot():
    for arg1, arg2 in arg_pairs():
